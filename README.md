...
| 二维码交叉验证 | 本地读取发票二维码，将号码、日期和金额与 OCR 独立核对；不冒充税务验真 |
...
1. 关键字段完整、格式正确、图像质量通过、二维码交叉一致、财务勾稽通过、
   无重复、未达到大额阈值
   → `机器预审通过`
2. 缺字段、格式/勾稽错误、二维码冲突、低质量图像、本次识别批内重复
   → `待人工复核（高风险）`
3. 税号警告或达到 `HIGH_VALUE_REVIEW_AMOUNT`
   → `待人工复核（中风险）`
4. 其余机器预审通过记录按“文件哈希 + 页码 + 发票号码”做逐张确定性抽样
...
覆盖 token 缓存/自动刷新、二维码证据、图像质量、信用代码校验、明细与大写
金额勾稽、红字发票、逐张抽样、OCR 缓存、单页重试、原值分离、鉴权和 CSRF。
